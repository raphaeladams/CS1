...
class Switch {
  ...
  Switch(int x, int y, int switchHeight, int switchWidth) {
    this.x = x;
    this.y = y;
    this.switchHeight = switchHeight;
    this.switchWidth = switchWidth;
    this.on = false;
    this.lightsTurnedOn = new Light[numLights];
  }
}
...
void setup() {
  ...
  for(int lightNum = 0; lightNum < lights.length; lightNum++) {
    float x = (spaceBetweenLights * (lightNum+1)) + (lightSize*lightNum) + lightSize/2;
...
void drawSwitch(Switch aSwitch) {
  ...
  rect(aSwitch.x, aSwitch.y, aSwitch.switchWidth, aSwitch.switchHeight);
  ...
  fill(0);
  text(numList, aSwitch.x + aSwitch.switchWidth/2, aSwitch.y + aSwitch.switchHeight/2);
}
...
void updateLights() {
  ...
  for(int lightNum = 0; lightNum < lights.length; lightNum++) {
    ...
    for(int switchNum = 0; switchNum < switches.length; switchNum++) {
      ...
      if(currentSwitch.on && currentSwitch.lightsTurnedOn[currentLight.num] == currentLight) {
        on = true;
...
void mouseClicked() {
  ...
  for(int switchNum = 0; switchNum < switches.length; switchNum++) {
    ...
    && mouseX <= switches[switchNum].x + switches[switchNum].switchWidth
    && mouseY >= switches[switchNum].y
    && mouseY <= switches[switchNum].y + switches[switchNum].switchHeight) {
